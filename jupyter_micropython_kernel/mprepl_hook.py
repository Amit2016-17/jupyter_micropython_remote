try:
    import os, io, gc, select, ustruct as struct, micropython, sys, uhashlib, ubinascii
except ImportError:
    pass
gc.collect()
...
class MpRepl:
    ...
    class RemoteFile(io.IOBase):
        ...
        def close(self):
            if self.fd is None:
                return
            self.cmd.begin(MpRepl.CMD_CLOSE)
            self.cmd.wr_int32(self.fd)
            self.cmd.end()
            self.fd = None

        def read(self, n):
            self.cmd.begin(MpRepl.CMD_READ)
            self.cmd.wr_int32(self.fd)
            self.cmd.wr_int32(n)
        ...
        def write(self, buf):
            self.cmd.begin(MpRepl.CMD_WRITE)
            self.cmd.wr_int32(self.fd)
        ...
        def seek(self, n):
            self.cmd.begin(MpRepl.CMD_SEEK)
            self.cmd.wr_int32(self.fd)
            self.cmd.wr_int32(n)
            n = self.cmd.rd_int32()
            self.cmd.end()
    ...
    class RemoteFS:
        ...
        def stat(self, path):
            self.cmd.begin(MpRepl.CMD_STAT)
        ...
        def ilistdir(self, path):
            self.cmd.begin(MpRepl.CMD_ILISTDIR_START)
            self.cmd.wr_str(self.path + path)
            self.cmd.end()

            def ilistdir_next():
                while True:
                    self.cmd.begin(MpRepl.CMD_ILISTDIR_NEXT)
        ...
        def open(self, path, mode):
            self.cmd.begin(MpRepl.CMD_OPEN)
            self.cmd.wr_str(self.path + path)
            ...
            if fd < 0:
                raise OSError(-fd)
            return MpRepl.RemoteFile(self.cmd, fd, mode.find('b') == -1)

    @staticmethod
    def mount(use_second_port):
        print(use_second_port)
        MpRepl.RemoteCommand.use_second_port = use_second_port
        os.mount(MpRepl.RemoteFS(), '/remote')
        os.chdir('/remote')


def exit(code=0):
    cmd = MpRepl.RemoteCommand()
    cmd.begin(MpRepl.CMD_EXIT)
    cmd.wr_int32(code)
    cmd.end()
